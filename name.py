...
class MkChineseName:
    ...
    first_names=[]
    def __init__(self):
        self.last_names = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许',
        ...
        self.first_names = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地', '为',
                   ...
                   '乾', '坤']
    def random_name(self,size=1, chars=string.ascii_letters + string.digits):
        return ''.join(random.choice(chars) for _ in range(size))

    def first_name(self,size=2, ln=None, fn=None):
       _lst = []
       for i in range(size):
            _item = self.random_name(1, fn)
            if ln:
                while _item in ln:
                    _item = self.random_name(1, fn)
                _lst.append(_item)
    ...
    def last_name(self,size=1, names=None):
        return self.random_name(size, names)


    def full_name(self,lns, fns):
        _last = self.last_name(1, lns)
        return "{}{}".format(_last, self.first_name(random.randint(1, 2), _last, fns))


    def mkname(self):
        return (self.full_name(self.last_names, self.first_names))